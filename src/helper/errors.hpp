...
public:
  static void error(const Lexer::Token *token, const std::string &msg,
                    Lexer &lexer) {
...
private:
  static void printErrorMessage(const Lexer::Token *token,
                                const std::string &msg) {
    std::cout << termcolor::red << "Error" << termcolor::reset
              << ": [line: " << termcolor::bright_blue << token->line
              << termcolor::reset << ", column: " << termcolor::bright_blue
              << token->column << termcolor::reset << "] " << termcolor::yellow
  ...
  static void printErrorLocation(const Lexer::Token *token, Lexer &lexer) {
    ...
    std::cout << "    " << token->line << " | "
              << lineContent.substr(0, lineEnd) << std::endl;
    std::cout << "      |" << std::string(token->column, ' ') << termcolor::red
